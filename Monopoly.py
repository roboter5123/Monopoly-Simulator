...
def chance(position, positions, log):
    ...
    if draw == 1:
        ...
        if position >= 35 or position < 5:
            ...
            log.write(f"Chance card. Go to 35. ")
            position = 35
...
logname = f"log_{date.strftime('%H-%M-%S')}.txt"
log = open(logname, "x")
positions = board()
...
for rounds in range(0, max_rounds):
    ...
    log.write(f"Position before dice throw: {position} ")
    dice = random.randint(2, 12)
    log.write(f"Dice: {dice} ")
    position = overflow(max_position, position, dice)
    log.write(f"Position after dice throw, before chance/community/Go to jail: {position} ")
    ...
    log.write(f"Position at the end of the turn: {position}\n")
...
log.write(f"\n\n Which position how often: {positions}\n")
log.close()
